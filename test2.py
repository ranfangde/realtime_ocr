...
# get_ipython().run_line_magic('pip', 'install -q "pyclipper>=1.2.1" "shapely>=1.7.1" tqdm')
...
def run_paddle_ocr(source=0, flip=False, use_popup=False, skip_first_frames=0):
    ...
    try:
        ...
        # 设置窗口的初始大小，例如宽度800，高度600
        cv2.resizeWindow('Output Image', 800, 600)
        while True:
            # Grab the frame.
            frame = player.next()
            if frame is None:
            ...
            if use_popup:
                ...
                #display.display(i)
                draw_img = cv2.cvtColor(draw_img, cv2.COLOR_RGB2BGR)
                cv2.imwrite('output_image.jpg', draw_img)  # 保存图像到文件
                cv2.imshow('Output Image', draw_img)

                # cv2.waitKey()负责处理窗口的消息事件, 1表示等待1毫秒
                if cv2.waitKey(10) & 0xFF == ord('q'):
                    break  # 如果按下 'q' 键则退出循环
                #draw_img = cv2.cvtColor(draw_img, cv2.COLOR_RGB2BGR)
                # cv2.imshow('Output Image', draw_img)  # 在窗口中显示图像
